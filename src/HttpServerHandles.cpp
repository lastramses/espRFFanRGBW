...
{
	if (LittleFS.exists("/index.html") == false)
	{ //use default conf page in AP mode if index.html does not exist
		String rootPage = "<!DOCTYPE html><html lang=\"en\">"
						  ...
						  "<tr><td>mac:</td><td>" +
						  WiFi.macAddress() + "</td><tr>"
											  "<tr><td>host</td><td><input type=\"text\" name=espHost value=" +
						  espHost + "></td></tr>"
...
void httpServerHandleGetData()
{
	...
	else if (httpServer.hasArg("confSunrise") == true)
	{
		...
		{
			jsonDeviceData = jsonDeviceData + "{\"Field\":\"stSunriseSimAct" + iSunRiseConf + "\",\"Data\":\"" + String(sunriseSim[iSunRiseConf].getStSunriseSimEna()) + "\"},"
				"{\"Field\":\"stSunriseSimDaysAct" + iSunRiseConf + "\",\"Data\":\"" + String(sunriseSim[iSunRiseConf].getTiSunriseDaysEna()) + "\"},"
				"{\"Field\":\"tiSunriseSimSHrStart" + iSunRiseConf + "\",\"Data\":\"0" + String(sunriseSim[iSunRiseConf].getTiSunriseHrStrt()) + "\"},"
				"{\"Field\":\"tiSunriseSimSMinStart" + iSunRiseConf + "\",\"Data\":\"0" + String(sunriseSim[iSunRiseConf].getTiSunriseMinStrt()) + "\"},"
				"{\"Field\":\"tiRampOnDurMin" + iSunRiseConf + "\",\"Data\":\"" + String(sunriseSim[iSunRiseConf].getTiRampOnDurMin()) + "\"},"
				"{\"Field\":\"tiStayOnDurMin" + iSunRiseConf + "\",\"Data\":\"" + String(sunriseSim[iSunRiseConf].getTiStayOnDurMin()) + "\"},"
				"{\"Field\":\"rgbRampEnd" + iSunRiseConf + "\",\"Data\":\"#" + String(sunriseSim[iSunRiseConf].hsl2rgb(sunriseSim[iSunRiseConf].getHSLRampEnd()), HEX) + "\"},"
				"{\"Field\":\"rgbEnd" + iSunRiseConf + "\",\"Data\":\"#" + String(sunriseSim[iSunRiseConf].hsl2rgb(sunriseSim[iSunRiseConf].getHSLEnd()), HEX) + "\"},";
		}
		jsonDeviceData = jsonDeviceData.substring(0,jsonDeviceData.length()-1) + "]";
		httpServer.send(200, "text/html", jsonDeviceData);
	}
...
void httpServerHandleSetData()
{
	...
	else if (httpServer.hasArg("saveSunriseConf") == true && httpServer.arg("saveSunriseConf") == "Save")
	{
		...
		for (uint8_t iSunRiseConf = 0; iSunRiseConf < sizeof(sunriseSim)/sizeof(sunriseSim[0]); ++iSunRiseConf)
		{
			...
			if (httpServer.arg("stSunriseSimAct" + String(iSunRiseConf)) == "on")
			...
			if (httpServer.hasArg("stDaySunEna" + String(iSunRiseConf)))
				tiSunriseDaysAct |= 0b00000001;
			if (httpServer.hasArg("stDayMonEna" + String(iSunRiseConf)))
				tiSunriseDaysAct |= 0b00000010;
			if (httpServer.hasArg("stDayTueEna" + String(iSunRiseConf)))
				tiSunriseDaysAct |= 0b00000100;
			if (httpServer.hasArg("stDayWedEna" + String(iSunRiseConf)))
				tiSunriseDaysAct |= 0b00001000;
			if (httpServer.hasArg("stDayThuEna" + String(iSunRiseConf)))
				tiSunriseDaysAct |= 0b00010000;
			if (httpServer.hasArg("stDayFriEna" + String(iSunRiseConf)))
				tiSunriseDaysAct |= 0b00100000;
			if (httpServer.hasArg("stDaySatEna" + String(iSunRiseConf)))
				tiSunriseDaysAct |= 0b01000000;
			configFile.write(tiSunriseDaysAct);

			String tiSunriseSimStartTime = httpServer.arg("tiSunriseSimStartTime" + String(iSunRiseConf));
			...
			uint8_t tiRampOnDurMin = strToInt(httpServer.arg("tiRampOnDurMin" + String(iSunRiseConf)));
			uint8_t tiStayOnDurMin = strToInt(httpServer.arg("tiStayOnDurMin" + String(iSunRiseConf)));
			configFile.write(tiHrStrt);
			configFile.write(tiMinStrt);
			configFile.write(tiRampOnDurMin);
			configFile.write(tiStayOnDurMin);
			uint32_t hslRampEnd = sunriseSim[iSunRiseConf].rgb2hsl(httpServer.arg("rgbRampEnd" + String(iSunRiseConf)));
			uint32_t hslEnd = sunriseSim[iSunRiseConf].rgb2hsl(httpServer.arg("rgbEnd" + String(iSunRiseConf)));
			configFile.write(0);
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
	...
	case WStype_TEXT:
	{
		...
		if (stRGBWAct == fsON && colourData.substring(0, 1) == "#")
		{
			...
			rgbwWeb = sunriseSim[0].rgb2rgbw(rgbwWeb);
